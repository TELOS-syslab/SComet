...
if [ -n "${core_ids[0]}" ]; then
    ...
    TBENCH_MINSLEEPNS=10000 real_time=1 taskset -c $core_ids_string ./mttest_integrated -j${NTHREADS} \
    mycsba masstree"
else
    cmd="TBENCH_QPS=${QPS} TBENCH_MAXREQS=${MAXREQS} TBENCH_WARMUPREQS=${WARMUPREQS} \
    TBENCH_MINSLEEPNS=10000 real_time=1 ./mttest_integrated -j${NTHREADS} \
    mycsba masstree"
fi
echo $cmd
