cd /home/wjy/spec2017/benchspec/CPU/520.omnetpp_r/run/run_base_refrate_mytest-m64.0000
...
for i in $(seq 0 $((num_threads - 1))); do
    ...
    if [ -n "$core_str" ]; then
        cmd="taskset -c $core_str ./omnetpp_r_base.mytest-m64 -c General -r 0"
    else
        cmd="./omnetpp_r_base.mytest-m64 -c General -r 0"
    fi

    eval "$cmd" > omnetpp_${i}.out  2>> omnetpp_${i}.err  &
done
