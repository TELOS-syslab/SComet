cd /home/wjy/spec2017/benchspec/CPU/508.namd_r/run/run_base_refrate_mytest-m64.0000
...
for i in $(seq 0 $((num_threads - 1))); do
    ...
    if [ -n "$core_str" ]; then
        cmd="taskset -c $core_str ./namd_r_base.mytest-m64 --input apoa1.input --output apoa1.ref.output --iterations 65"
    else
        cmd="./namd_r_base.mytest-m64 --input apoa1.input --output apoa1.ref.output --iterations 65"
    fi

    eval "$cmd" > namd_${i}.out  2>> namd_${i}.err  &
done
