cd /home/wjy/spec2017/benchspec/CPU/521.wrf_r/run/run_base_refrate_mytest-m64.0000
...
for i in $(seq 0 $((num_threads - 1))); do
    ...
    if [ -n "$core_str" ]; then
        cmd="taskset -c $core_str ./wrf_r_base.mytest-m64"
    else
        cmd="./wrf_r_base.mytest-m64"
    fi

    eval "$cmd" > wrf_${i}.out  2>> wrf_${i}.err  &
done
