cd /home/wjy/spec2017/benchspec/CPU/505.mcf_r/run/run_base_refrate_mytest-m64.0000
...
for i in $(seq 0 $((num_threads - 1))); do
    ...
    if [ -n "$core_str" ]; then
        cmd="taskset -c $core_str ./mcf_r_base.mytest-m64 inp.in"
    else
        cmd="./mcf_r_base.mytest-m64 inp.in"
    fi

    eval "$cmd" > mcf_${i}.out  2>> mcf_${i}.err  &
done
