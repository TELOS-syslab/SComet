cd /home/wjy/spec2017/benchspec/CPU/500.perlbench_r/run/run_base_refrate_mytest-m64.0000
...
for i in $(seq 0 $((num_threads - 1))); do
    ...
    if [ -n "$core_str" ]; then
        cmd="taskset -c $core_str ./perlbench_r_base.mytest-m64 -I./lib checkspam.pl 2500 5 25 11 150 1 1 1 1"
    else
        cmd="./perlbench_r_base.mytest-m64 -I./lib checkspam.pl 2500 5 25 11 150 1 1 1 1"
    fi

    eval "$cmd" > perlbench_${i}.out  2>> perlbench_${i}.err  &
done
