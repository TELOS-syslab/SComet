cd /home/wjy/spec2017/benchspec/CPU/541.leela_r/run/run_base_refrate_mytest-m64.0000
...
for i in $(seq 0 $((num_threads - 1))); do
    ...
    if [ -n "$core_str" ]; then
        cmd="taskset -c $core_str ./leela_r_base.mytest-m64 ref.sgf"
    else
        cmd="./leela_r_base.mytest-m64 ref.sgf"
    fi

    eval "$cmd" > leela_${i}.out  2>> leela_${i}.err  &
done
