cd /home/wjy/spec2017/benchspec/CPU/507.cactuBSSN_r/run/run_base_refrate_mytest-m64.0000
...
for i in $(seq 0 $((num_threads - 1))); do
    ...
    if [ -n "$core_str" ]; then
        cmd="taskset -c $core_str ./cactusBSSN_r_base.mytest-m64 spec_ref.par"
    else
        cmd="./cactusBSSN_r_base.mytest-m64 spec_ref.par"
    fi

    eval "$cmd" > cactusBSSN_${i}.out  2>> cactusBSSN_${i}.err  &
done
