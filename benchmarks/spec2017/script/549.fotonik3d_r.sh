cd /home/wjy/spec2017/benchspec/CPU/549.fotonik3d_r/run/run_base_refrate_mytest-m64.0000
...
for i in $(seq 0 $((num_threads - 1))); do
    ...
    if [ -n "$core_str" ]; then
        cmd="taskset -c $core_str ./fotonik3d_r_base.mytest-m64"
    else
        cmd="./fotonik3d_r_base.mytest-m64"
    fi

    eval "$cmd" > fotonik3d_${i}.out  2>> fotonik3d_${i}.err  &
done
