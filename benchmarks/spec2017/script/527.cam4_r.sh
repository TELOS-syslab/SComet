cd /home/wjy/spec2017/benchspec/CPU/527.cam4_r/run/run_base_refrate_mytest-m64.0000
...
for i in $(seq 0 $((num_threads - 1))); do
    ...
    if [ -n "$core_str" ]; then
        cmd="taskset -c $core_str ./cam4_r_base.mytest-m64"
    else
        cmd="./cam4_r_base.mytest-m64"
    fi

    eval "$cmd" > cam4_${i}.out  2>> cam4_${i}.err  &
done
