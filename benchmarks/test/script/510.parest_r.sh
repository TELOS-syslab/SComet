cd /home/wjy/spec2017/benchspec/CPU/510.parest_r/run/run_base_refrate_mytest-m64.0000
...
for i in $(seq 0 $((num_threads - 1))); do
    ...
    if [ -n "$core_str" ]; then
        cmd="taskset -c $core_str ./parest_r_base.mytest-m64 ref.prm"
    else
        cmd="./parest_r_base.mytest-m64 ref.prm"
    fi

    eval "$cmd" > parest_${i}.out  2>> parest_${i}.err  &
done
