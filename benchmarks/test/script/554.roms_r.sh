cd /home/wjy/spec2017/benchspec/CPU/554.roms_r/run/run_base_refrate_mytest-m64.0000
...
for i in $(seq 0 $((num_threads - 1))); do
    ...
    if [ -n "$core_str" ]; then
        cmd="taskset -c $core_str ./roms_r_base.mytest-m64 < ocean_benchmark2.in.x"
    else
        cmd="./roms_r_base.mytest-m64 < ocean_benchmark2.in.x"
    fi

    eval "$cmd" > roms_${i}.out  2>> roms_${i}.err  &
done
