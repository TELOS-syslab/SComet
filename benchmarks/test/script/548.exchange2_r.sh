cd /home/wjy/spec2017/benchspec/CPU/548.exchange2_r/run/run_base_refrate_mytest-m64.0000
...
for i in $(seq 0 $((num_threads - 1))); do
    ...
    if [ -n "$core_str" ]; then
        cmd="taskset -c $core_str ./exchange2_r_base.mytest-m64 6"
    else
        cmd="./exchange2_r_base.mytest-m64 6"
    fi

    eval "$cmd" > exchange2_${i}.out  2>> exchange2_${i}.err  &
done
