...
class program_mgr:
    def __init__(self, config_path=None, regular_update=False, nopartition=False, log_prefix="OSML", manage=True, enable_models=False,training=True, ip = None):
        self.ip = ip
        ...
        if self.enable_models:
            init_models()
        self.BEs = BEs(self.get_COS_id(), self.ip)
    ...
    def monitor(self):
        ...
        run_on_node(self.ip, f"mkdir -p {self.tmp_dir}")

        pqos_file = f"{self.tmp_dir}/pqos.all"
        cmd_pqos = "nohup pqos -I -i 1 -m all:0-{} &".format(N_CORES - 1)
        cmd_pqos_redirect = f"{cmd_pqos} > {pqos_file} 2>&1"
        run_on_node(self.ip, cmd_pqos_redirect)
    ...
    def end_log_thread(self):
        ...
        run_on_node(self.ip, f"mkdir -p {ROOT}/logs/")
    ...
    def conduct_allocation(self):
        ...
        def allocate_core(p, core_used):
            try:
                if len(core_used) == 0:
                    return
                cmd_get_threads = f"ps -L -p {p.pid} -o tid="
                proc = run_on_node(self.ip, cmd_get_threads)
                out, err = proc.communicate()
                if proc.returncode == 0:
                    tids = [int(t) for t in out.decode().split()]
                    for t in tids:
                        cmd_bind = f"taskset -pc {','.join(map(str, core_used))} {t}"
                        run_on_node(self.ip, cmd_bind)
                # ps = psutil.Process(p.pid)
                # ps_threads = ps.threads()
                # for t in ps_threads:
                    # os.sched_setaffinity(t.id, core_used)
                    # cmd = f"taskset -pc {','.join(map(str, core_used))} {t.id}"
                    # run_on_node(self.ip, cmd)
            except:
                pass
        ...
        if not self.BEs.empty():
            try:
                for BE_name in self.BEs.BEs:
                    ...
                    pid = self.BEs.pid[BE_name]
                    # 获取远程线程ID列表
                    cmd_get_threads = f"ps -L -p {pid} -o tid="
                    proc = run_on_node(self.ip, cmd_get_threads)
                    out, err = proc.communicate()
                    if proc.returncode == 0:
                        # 解析线程ID
                        tids = [int(t) for t in out.decode().split()]
                        for t in tids:
                            cmd_bind = f"taskset -pc {','.join(map(str, core_used['BE']))} {t}"
                            run_on_node(self.ip, cmd_bind)
                # ps_threads = ps.threads()
                # for t in ps_threads:
                    # os.sched_setaffinity(t.id, core_used["BE"])
                    # cmd = f"taskset -pc {','.join(map(str, core_used['BE']))} {t.id}"
                    # run_on_node(self.ip, cmd)
            except:
                pass
...
class program:
    def __init__(self, name, proc_id, COS_id, RPS, RPS_str, n_threads, launch_time, end_time, regular_update, ip):
        self.ip = ip
        self.name = name
        self.pid = None
        self.pid_str = None
    ...
    def launch(self, core_start=None, core_len=None, way_start=None, way_len=None):
        ...
        for cmd in cmds:
            ...
            remote_cmd = taskset_str + cmd.format(RPS=self.RPS, threads=self.n_threads)
            run_on_node(self.ip, remote_cmd)
    ...
    def end(self):
        ...
        if isinstance(self.pid, list):
            for pid in self.pid:
                ...
                run_on_node(self.ip, f"kill {pid}")
            else:
                # subprocess.call("kill {}".format(self.pid), shell=True)
                run_on_node(self.ip, f"kill {self.pid}")
        self.mode = MODE.Dead
    ...
    def get_pid(self):
        ...
        if False:
            ...
            proc = run_on_node(self.ip, f"pgrep -n -f {NAME_2_PNAME[self.name]}")
            outs, errs = proc.communicate()
            self.pid = int(outs.decode().strip())
    ...
    def monitor(self):
        ...
        pqos_file = f"{self.tmp_dir}/pqos.{self.pid_str_md5}"
        top_file = f"{self.tmp_dir}/top.{self.pid_str_md5}"
        ...
        if isinstance(self.pid, list):
            if self.name == "nginx":
                cmd_top = f'nohup top -d {TOP_INTERVAL} -b -w 512 | grep -E "COMMAND|nobody.*{self.name}" > {top_file} 2>&1 &'
            else:
                cmd_top = f'nohup top -d {TOP_INTERVAL} -b -w 512 | grep -E "COMMAND|{self.name}" > {top_file} 2>&1 &'
        else:
            cmd_top = f'nohup top -p {self.pid_str} -d {TOP_INTERVAL} -b -w 512 > {top_file} 2>&1 &'
        run_on_node(self.ip, cmd_top)
    ...
    def set_state(self, keys):
        ...
        if any([key in ["Virt_Memory", "Res_Memory", "CPU_Utilization", "Memory_Footprint"] for key in keys]):
            if isinstance(self.pid, int):
                ...
                mem_res = run_on_node(self.ip,
                                      f"tail -n {(len(self.pid) + 1) * 2} "
                                      f"{self.tmp_dir}top.{self.pid_str_md5}").communicate()[0].decode()
                if len(mem_res) == 0:
                    return False
        ...
        if any([key in ["IPC", "Misses", "LLC", "MBL"] for key in keys]):
            if self.name in ["nginx"]:
                ...
                pqos_res = run_on_node(self.ip, f"tail -n {(N_CORES + 2) * 2} "
                                                f"{self.tmp_dir}pqos.all").communicate()[0].decode().strip()
                ...
                pqos_res = run_on_node(self.ip, f"tail -n 3 "
                                                f"{self.tmp_dir}pqos.{self.pid_str_md5}").communicate()[0].decode()
        ...
        if any([key in ["Frequency"] for key in keys]):
            ...
            for i in core_list:
                ...
                freq_temp = re.findall(r"\d+\.?\d*", freq_line[i])
        ...
        if any([key in ["IPS"] for key in keys]):
            if IPS_update:
                try:
                    ...
                    IPS_res = run_on_node(self.ip,
                                          f"perf stat -p {self.pid} -e instructions sleep 0.05 2>&1 | "
                                          f"grep instructions").communicate()[0].decode()

                    IPS = int("".join(IPS_res.split()[0].split(",")))
...
class BEs:
    ...
    def remove(self, name):
        self.BEs.remove(name)
        self.status.pop(name)
        self.pid_str.pop(name)
        self.pid_str_md5.pop(name)
        self.pid.pop(name)
    ...
    def get_throughput(self,name):
        ...
        try:
            ...
            pqos_res = run_on_node(self.ip, f"tail -n 3 "
                                            f"{self.tmp_dir}pqos.{self.pid_str_md5[name]}").communicate()[0].decode()
            pqos_split = pqos_res.split()
