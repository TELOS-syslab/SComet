...
def initialize_hardware_isolation():
    ...
    try:
        ...
        # 验证cache_ways配置是否合理
        if sum(cache_ways) > total_ways:
            print(f"警告：配置的cache_ways总和({sum(cache_ways)})超过系统LLC总路数({total_ways})")
            print("自动调整cache_ways配置以匹配系统限制")
            # 按比例调整cache_ways
            ratio = total_ways / sum(cache_ways)
            adjusted_ways = [max(1, int(w * ratio)) for w in cache_ways]
            # 确保总和等于total_ways
            while sum(adjusted_ways) < total_ways:
                max_idx = adjusted_ways.index(max(adjusted_ways))
                adjusted_ways[max_idx] += 1
            while sum(adjusted_ways) > total_ways:
                min_idx = adjusted_ways.index(min(adjusted_ways))
                if adjusted_ways[min_idx] > 1:
                    adjusted_ways[min_idx] -= 1
                else:
                    break
            cache_ways[:] = adjusted_ways
            print(f"调整后的cache_ways: {cache_ways}")
...
def assign_container_to_cos(container_id, cos_id):
    try:
        ...
        # 启用PEBS加载延迟事件支持（根据文档要求）
        if cos_id == 0:  # LC任务，假设需要启用PEBS
            pebs_enable_cmd = f"sudo pqos -a 'pebs:0=enable'"
            subprocess.run(pebs_enable_cmd, shell=True, check=True)
            print(f"为 COS{cos_id} 容器启用 PEBS 加载延迟事件支持")
...
# 加载PMU事件（适配真实MEM_TRANS_RETIRED.LOAD_LATENCY_GT_X事件格式）
def load_pmu_events():
    event_list = []
    # 目标事件前缀，严格匹配MEM_TRANS_RETIRED.LOAD_LATENCY_GT_XXX格式
    TARGET_EVENT_PREFIX = "MEM_TRANS_RETIRED.LOAD_LATENCY_GT_"
                ...
                try:
                    ...
                    if "Events" in data.keys():
                        for e in data["Events"]:
                            ...
                            # 1. 严格匹配事件名称前缀
                            if not event_name.startswith(TARGET_EVENT_PREFIX):
                                continue
                            
                            # 2. 校验核心字段（适配真实事件格式）
                            # 必须包含EventCode和UMask（真实事件中用UMask而非UnitMask）
                            if not (e.get("EventCode") and e.get("UMask")):
                                print(f"警告：事件 {event_name} 缺少EventCode或UMask，跳过")
                                continue
                            
                            # 3. 校验PEBS支持（真实事件中PEBS通过PEBScounters和CollectPEBSRecord体现）
                            pebs_supported = (
                                e.get("PEBScounters") is not None and 
                                e.get("CollectPEBSRecord", "0") != "0"
                            )
                            if not pebs_supported:
                                print(f"警告：事件 {event_name} 不支持PEBS，跳过")
                                continue
                            
                            # 4. 校验事件未被废弃
                            if e.get("Deprecated") != "0":
                                print(f"警告：事件 {event_name} 已废弃，跳过")
                                continue
                            
                            # 5. 提取延迟阈值（从事件名称解析X值，如128）
                            try:
                                threshold = int(event_name.split("_")[-1])
                                if threshold < 4:  # 手册规定最小阈值为4周期
                                    print(f"警告：事件 {event_name} 阈值{threshold} < 4，跳过")
                                    continue
                            except ValueError:
                                print(f"警告：无法解析事件 {event_name} 的阈值，跳过")
                                continue
                            
                            # 6. 校验UMask格式（真实事件中为0x开头的16进制）
                            if not re.match(r"0x[0-9A-Fa-f]+", e["UMask"]):
                                print(f"警告：事件 {event_name} 的UMask格式错误，跳过")
                                continue
                            
                            # 7. 处理TakenAlone字段（真实事件中可能需要单独采集）
                            if e.get("TakenAlone") == "1":
                                print(f"注意：事件 {event_name} 需要单独采集（TakenAlone=1）")
    ...
    print(f"成功加载 {len(event_list)} 个符合格式的MEM_TRANS_RETIRED.LOAD_LATENCY_GT_X事件")
    return event_list

# 检查是否有冲突的PEBS事件正在运行
def check_running_pebs_events():
    try:
        output = subprocess.check_output("ps aux | grep ocperf | grep -v grep", shell=True, text=True)
        running_events = []
        for line in output.split('\n'):
            if "ocperf" in line and "-e" in line:
                # 提取-e参数后的事件列表
                e_index = line.index("-e") + 2
                if e_index < len(line):
                    events_part = line[e_index:].split()[0]
                    events = events_part.split(',')
                    running_events.extend(events)
        return running_events
    except Exception as e:
        print(f"检查运行中的PEBS事件失败: {e}")
        return []
...
def collect_lc_tail_latency(benchmark, be_core, test_time, latency_events, lc_task, lc_core, lc_threads, lc_rps):
    ...
    for event in latency_events:
        if 'LATENCY_GT_4' in event["EventName"].upper() and event.get("Deprecated") == "0":
            ge1_event = event
            break
    if not ge1_event:
...
def collect_be_pmu_events_dynamic(benchmark, be_core, test_time, latency_events, lc_task, lc_core, lc_threads, lc_rps):
    # 新增：采集前检查是否已有冲突事件启用（模拟硬件约束）
    running_pebs_events = check_running_pebs_events()
    if running_pebs_events and any("LOAD_LATENCY_GT" in e for e in running_pebs_events):
        print("错误：已有加载延迟 PEBS 事件启用，无法并行采集")
        return None
...
def parse_pmu_log(log_file):
    ...
    with open(log_file, 'r') as f:
        for line in f:
            ...
            # 按文档校验事件是否符合PEBS记录格式
            if "LOAD_LATENCY" in event_name:
                # 验证事件名称格式
                if not re.match(r".*LATENCY_GT_\d+.*", event_name):
                    print(f"警告：事件 {event_name} 格式不符合文档 PEBS 记录规范，跳过")
                    continue
                # 验证数据源字段（模拟文档中表21-14的检查）
                if "SOURCE" in counts_str and not re.search(r"SOURCE=\d+", counts_str):
                    print(f"警告：事件 {event_name} 的数据源字段格式异常，跳过")
                    continue
